...
from src.Service.undo_service import FunctionCall, Operation
...
class GradeService:
    ...
    def append_grade_to_student(self, grade):
        ...
        if DisciplineValidation.check_existence_of_discipline_id(grade.discipline_id_g, self.discipline_repo):
            if StudentValidation.check_existence_of_student_id(grade.student_id_g, self.students_repo):
            ...
            for d in self.discipline_repo:
                ...
                for g in self.grades.grade_list:
                    if self.students_repo[i].student_id == g.student_id_g and d.discipline_id == g.discipline_id_g:
                        sum_of_grades = sum_of_grades + int(g.grade_value)
    ...
    def sort_grades_list(self):
        ...
        :return: It doesn't return anything
        """
        self.grades.grade_list.sort(reverse=False, key=lambda x: x.discipline_id_g)
